...
<img id="title" src="images/personalLibrary.png" alt="Personal Library">
<script>
...
<div class="container">
    <header class="clearfix">
        <span class="support-note" style="text-align: center; font-size: 15pt; color: white">These are the books you have read! <br> Click on a book to read about it! Add the book to your favourites inside. (Best viewed in WebKit browsers)</span>
    </header>
    <div class="main">
...
<div id="loginModal" style="display:none; position:fixed; z-index:1500; left:0; top:0; width:100%; height:100%; overflow:auto; background-color:rgba(0,0,0,0.4);">
    <div style="background-color:#fefefe; margin:15% auto; padding:20px; border:1px solid #888; width:80%; max-width:450px; position: relative;">
        ...
        <p>You must be logged in to view your personal library. Please <a class="first after" href="<?= ($BASE) ?>/login">log in</a> or <a class="first after" href="<?= ($BASE) ?>/register">register</a>.</p>
    </div>
</div>
...
<script>
    async function fetchBooksAndCreateISBNArray(isLoggedIn) {
        ...
        try {
            const response = await fetch('<?= ($BASE) ?>/api/user-read'); // Fetch the books data
    ...
    document.addEventListener('DOMContentLoaded', async () => {
                    ...
                    <div class="bk-book book-${bookCounter}">
                        ...
                        <div class="bk-page">
                            <div class="bk-content bk-content-current">
                                ...
                                <button class="make-favourite" data-isbn="${isbn}">Add to Favourites</button>
                            </div>
        ...
        Promise.all(fetchPromises).then(() => {
            ...
            document.querySelectorAll('.make-favourite').forEach(button => {
                button.addEventListener('click', function() {
                    const isbn = this.getAttribute('data-isbn'); // Get the ISBN from the button's data attribute
                    addToFavourites(isbn); // Call the function to add the book to favourites
                });
            });
        }).catch(error => {
            console.error('Error initializing books:', error);
        });
        function addToFavourites(isbn) {
            $.ajax({
                url: '<?= ($BASE) ?>/makeFavourite', // Replace '<?= ($BASE) ?>' with your actual base URL if necessary
                type: 'POST',
                data: { isbn: isbn },
                success: function(response) {
                    ...
                    if(result.success) {
                        ...
                        alert('Error adding book to favourites.');
                    }
                },
                error: function() {
